...
- [ ] **(RISK) Kill queued as feature is disabled (the `killSwitchMonitor` gate):** open controlled app
  ...
  feature (log shows "ignoring queued kill … (feature disabled)").
...
- [ ] **Copy/Share at the 10 000-line preset with a long-running Debug session** → no crash
  (watch for `TransactionTooLargeException`; this is a known Minor to confirm in practice).
- [ ] **Clear** empties the on-screen buffer.
