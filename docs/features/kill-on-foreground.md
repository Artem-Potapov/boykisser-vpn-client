...
- **Kill and revive interact with auto-failover's rotation.** A kill landing during a rotation (`ROTATING`) is **deferred** exactly like one landing during a revive — `shouldDeferKillDuringTransition` is the single rule covering both — and replayed if the rotation commits `CONNECTED`. A rotation has a **third** exit, though: if it *gives up*, `giveUpRotationLocked` **drops** the deferred kill (clearing `pendingKillLabel` at the top of the funnel) and posts the "VPN is still on" notice (id **1106**, on this feature's own exposure channel) naming the app. Replaying it up to `rotationWindowMs` later instead would tear down a just-restored tunnel and blame an app the user closed long ago; dropping it *silently* would leave the kill-switch quietly non-functioning while the listed app is still riding the tunnel — hence the notice rather than either. See [auto-failover.md](auto-failover.md).
- `killTunnel` **stops** (not pauses) the failover monitor, because pausing preserves the consecutive-failure count and it would trip instantly on revive; `reviveTunnel`'s success path re-applies failover preferences to bring the monitor back, and clears any leftover give-up state + its `1105` alert.
- **A deferral also has a fourth leg: the listed app leaving before the transition commits WITHDRAWS it.** `reviveTunnel` calls `withdrawDeferredKillLocked` on every leave callback (pure rule `deferredKillToWithdraw`), before and independently of the revive decision. Without it, opening and closing a listed app during a rotation parked the session in `PAUSED` — no TUN, every app on the clear network — for an app that had gone, with no automatic recovery, because this monitor is edge-triggered and never re-fires that leave. Two things make it work and must not be undone: the withdrawal takes **no tunnel state** (a leave arriving during a bring-up cannot run until the transition has already committed `CONNECTED`, since `tunnelOpScope` is `limitedParallelism(1)` and `bringUpTunnel` does not suspend), and the two commits now **observe** `pendingKillLabel` rather than consuming it, so `replayDeferredKill` resolves it at execution time and a withdrawal queued ahead of it still has something to clear. **It posts nothing** — in particular not 1106, which is for a kill dropped while the listed app is *still tunneled*; here the app has left, so there is no app riding the tunnel to warn about and alerting would report a failure that did not occur. Contrast the give-up drop above, which *does* alert precisely because the app is still tunneled. See [auto-failover.md](auto-failover.md).
